...
<head>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            .dropdown-toggle .flex.items-center span {
                display: none;
            }
            .dropdown-menu {
    ...
    <div class="main-wrapper">
        <main class="flex-1 flex flex-col">
            <header class="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-4 md:px-10 shrink-0">
                <div><h2 class="serif text-xl md:text-2xl text-gray-800 italic">Table Management</h2></div>
            ...
            <div class="p-4 md:p-10 max-w-7xl w-full mx-auto">
                ...
                <div class="bg-white border border-gray-100 p-4 md:p-10 shadow-sm relative overflow-hidden">
                    ...
                    <form method="GET" action="/reservation" id="filterForm" class="mb-8">
                        <div class="filter-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                            <div>
                            ...
                            <div>
                                <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Table Filter</label>
                            ...
                            <div>
                                <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Room Filter</label>
                            ...
                            <div class="flex items-end gap-2">
                                <button type="submit" class="accent-gold text-white text-[11px] font-bold uppercase tracking-[0.3em] px-4 py-2 h-10 hover:bg-black transition whitespace-nowrap">
                                    Apply Filters
                                </button>
                                <button type="button" id="resetFiltersBtn" class="border border-gray-300 text-gray-500 text-[11px] font-bold uppercase tracking-[0.3em] px-4 py-2 h-10 hover:bg-gray-100 transition whitespace-nowrap">
                                    Reset
                                </button>
                            </div>
                        </div>
                        ...
                        @else
                            <div class="space-y-4">
                                    ...
                                    @foreach($spaces as $space)
                                        <label class="relative cursor-pointer group space-card {{ !$space->is_available ? 'disabled opacity-50' : '' }}">
                                            ...
                                            <div class="border-2 {{ $space->is_available ? 'border-gray-100 hover:border-gold' : 'border-red-200 bg-gray-50' }} overflow-hidden transition-all duration-500 peer-checked:border-gold peer-checked:shadow-lg">
                                                <div class="aspect-[4/3] bg-gray-100 flex flex-col items-center justify-center text-gray-400 relative">
                                                ...
                                                <div class="p-4">
                                                    ...
                                                    @if($space->is_available)
                                                        <span class="text-[9px] text-green-600 uppercase tracking-wider block mt-1">
                                                            ✓ Available
                                                        </span>
                                                    @endif
                                                    <p class="text-[9px] uppercase tracking-[0.2em] text-gold mt-2 flex items-center gap-2">
                                ...
                                @if($spaces->lastPage() > 1)
                                    <div class="flex flex-wrap justify-center items-center gap-2 mt-8 pt-4 border-t border-gray-100 pagination-flex">
                                        @if($spaces->onFirstPage())
                                            <span class="px-3 py-2 bg-gray-100 text-gray-400 rounded-lg text-sm cursor-not-allowed">← Previous</span>
                                        @else
                                            <a href="{{ $spaces->appends(request()->query())->previousPageUrl() }}" 
                                               class="px-3 py-2 bg-white border border-gray-300 text-gray-600 rounded-lg text-sm hover:border-gold hover:text-gold transition">
                                                ← Previous
                                            </a>
                                        @endif
                                        ...
                                        <div class="flex flex-wrap gap-1">
                                            @foreach(range(1, $spaces->lastPage()) as $page)
                                                ...
                                                @else
                                                    <a href="{{ $spaces->appends(request()->query())->url($page) }}" 
                                                       class="px-3 py-2 bg-white border border-gray-300 text-gray-600 rounded-lg text-sm hover:border-gold hover:text-gold transition">
                                                        {{ $page }}
                                                    </a>
                                                @endif
                                        ...
                                        @if($spaces->hasMorePages())
                                            <a href="{{ $spaces->appends(request()->query())->nextPageUrl() }}" 
                                               class="px-3 py-2 bg-white border border-gray-300 text-gray-600 rounded-lg text-sm hover:border-gold hover:text-gold transition">
                                                Next →
                                            </a>
                                        @else
                                ...
                                <p class="text-[10px] text-gray-400 mt-4 italic">👆 Click on any available table/room above to select it</p>
                            </div>

                            <div class="flex justify-end gap-4 pt-8">
                                <button type="button" onclick="document.getElementById('reservationForm').reset()" 
                                    class="border border-gray-300 text-gray-500 text-[11px] font-bold uppercase tracking-[0.3em] px-6 md:px-8 py-3 md:py-4 hover:bg-gray-100 transition">
                                    Clear
                                </button>
                                <button type="submit" 
                                    class="accent-gold text-white text-[11px] font-bold uppercase tracking-[0.3em] px-6 md:px-12 py-3 md:py-4 hover:bg-black transition-all duration-500">
                                    Confirm Adventure
                                </button>
                            </div>
                        @endif
